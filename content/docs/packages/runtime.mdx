...
The `registry` field is your **loop catalog** client (`LoopRegistry` in types) — definitions loaded from disk, HTTP, or Better Data, not the Commerce Gateway Registry.
...
listOpen(loopId: string): Promise<LoopInstance[]>
