...
description: "AI actor adapter for Google Gemini — wire Gemini 1.5 Pro/Flash as a governed actor in any operational loop."
section: Packages
taxonomy: provider
---
...
`@loop-engine/adapter-gemini` wraps Google's Gemini API as a Boss Loops AI actor using the `@google/generative-ai` SDK. Unlike OpenAI-compatible adapters, Gemini uses Google's native SDK and response shape. The adapter applies the same governance model as other Boss Loops AI adapters: guard enforcement in runtime and consistent actor audit fields.
