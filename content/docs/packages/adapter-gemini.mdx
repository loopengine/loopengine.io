...
`@loop-engine/adapter-gemini` wraps Google's Gemini API as a Boss AI actor using the `@google/generative-ai` SDK. Unlike OpenAI-compatible adapters, Gemini uses Google's native SDK and response shape. The adapter applies the same governance model as other Boss AI adapters: guard enforcement in runtime and consistent actor audit fields.
