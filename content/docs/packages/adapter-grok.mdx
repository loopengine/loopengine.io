...
`@loop-engine/adapter-grok` wraps xAI's Grok API as a Boss Loops AI actor. Grok uses an OpenAI-compatible API, and this adapter uses the `openai` npm package pointed at `https://api.x.ai/v1`. It follows the same governance model as `adapter-anthropic` and `adapter-openai`: identical guard enforcement and the same audit trail shape.
