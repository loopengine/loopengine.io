...
**Terminology:** In product docs we call this the **loop catalog** (versioned loop definitions for Boss). The npm package remains `@loop-engine/registry-client` and TypeScript types still use the name `LoopRegistry`. That is **not** the [Commerce Gateway Registry](https://commercegateway.io/registry) (gateway discovery and verification).
