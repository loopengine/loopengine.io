...
description: Use OpenClaw as the human approval channel for Boss Loops and route approval gates through WhatsApp, Telegram, Slack, or Discord.
section: Examples
---
...
| --- | --- |
| **Provider** | Optional — agent reasoning may live in OpenClaw; governed transitions still pass through Boss Loops |
...
```text
Provider (optional) → Loop + Guards → Channel (OpenClaw) → Integration → Evidence
```

## Overview

OpenClaw handles delivery and replies across messaging channels, while Boss Loops owns state transitions, actor constraints, and guard decisions.
