...
Most of the `@loop-engine/*` family is published in the `1.0.0-rc.0` cohort. A small number of adapters may remain pre-1.0 while their API shape stabilizes (for example, `@loop-engine/adapter-postgres@0.2.0`). Install the capability you need; cohort boundaries are explicit in the docs where they matter.
</Callout>
